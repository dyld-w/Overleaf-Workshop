...
export interface ProjectEntity {
    ...
    deletedDocs: Array<{ _id: string, name: string, deletedAt: string }>,
...
export class VirtualFileSystem extends vscode.Disposable {
    ...
    private ensureSocketLifecycleBridge() {
        ...
        //     // Show the modal once; then keep a sticky until reconnect.
    ...
    public onSocketLifecycle(handlers: { onDisconnected?: () => void; onReconnected?: () => void }): vscode.Disposable {
        ...
        if (handlers.onReconnected) {
            // Wrap user handler with modal/sticky cleanup
            subs.push(this.onDidReconnect(() => {
                this.retryConnection = 0;
                // Call user-provided handler after cleanup
    ...
    private get initializingPromise(): Promise<ProjectEntity> {
        ...
        // in initializingPromise() BEFORE calling remoteWatch():
    ...
    async _resolveUri(uri: vscode.Uri) {
        ...
        const [fileEntity, fileType, fileId] = (() => {
            for (const _type of Object.keys(FolderKeys)) {
                let entity = parentFolder[FolderKeys[_type]]?.find((e) => e.name === fileName);
                if (!fileName && _type === 'folder') { entity = parentFolder; }
                if (entity) return [entity, _type as FileType, entity._id];
            }
            return [];
        })();
    ...
    _resolveById(entityId: string, root?: FolderEntity, p?: string): {
        parentFolder: FolderEntity, fileEntity: FileEntity, fileType: FileType, path: string
    } | undefined {
        if (!this.root) throw vscode.FileSystemError.FileNotFound();
        root = root || this.root.rootFolder[0];
        p = p || '/';

        if (root._id === entityId) return { parentFolder: root, fileType: 'folder', fileEntity: root, path: p };
        for (const _type of Object.keys(FolderKeys)) {
            const key = FolderKeys[_type];
            if (key === 'folders') continue;
            const entity = root[key]?.find((e) => e._id === entityId);
            if (entity) return { parentFolder: root, fileType: _type as FileType, fileEntity: entity, path: p + entity.name };
        }
        for (const folder of root.folders) {
            const res = this._resolveById(entityId, folder, p + folder.name + '/');
            if (res) return res;
        }
        return undefined;
    }
    ...
    walk(filter: (entity: FileEntity) => boolean): { entity: FileEntity, path: string }[] {
        ...
        for (const folder of folders) {
            for (const [key, value] of Object.entries(FolderKeys)) {
                if (value === 'folders') {
                    folder.entity[value]?.forEach((e) => folders.push({ entity: e, path: folder.path + e.name + '/' }));
                }
                folder.entity[value]?.forEach((e) => {
                    (e as any)._type = key as FileType;
                    filter(e) && result.push({ entity: e, path: folder.path + e.name });
                });
            }
        }
        return result;
    ...
    private insertEntity(parentFolder: FolderEntity, fileType: FileType, entity: FileEntity) {
        ...
        if (index === undefined || index < 0) parentFolder[key]?.push(entity as any);
    }
    ...
    private removeEntityById(parentFolder: FolderEntity, fileType: FileType, entityId: string) {
        const key = FolderKeys[fileType];
        ...
        this.socket.updateEventHandlers({
            ...
            onFileCreated: (parentFolderId: string, type: FileType, entity: FileEntity) => {
                ...
                if (res) {
                    ...
                    this.notify([{ type: vscode.FileChangeType.Created, uri: this.pathToUri(entityPath) }]);
                }
            },
            ...
            onFileRemoved: (entityId: string) => {
                ...
                if (res) {
                    ...
                    this.notify([{ type: vscode.FileChangeType.Deleted, uri: this.pathToUri(res.path) }]);
                }
            },
            ...
            onFileChanged: (update: UpdateSchema) => {
                ...
                if (update.v === doc.version) {
                    ...
                    if (update.op && doc.remoteCache !== undefined) {
                        ...
                        const _doc = vscode.workspace.textDocuments.find((d) => d.uri.toString() === _uri);
                        if (_doc && !_doc.isDirty) { doc.localCache = content; }
                        doc.remoteCache = content;
                        this.isDirty = true;
                        this.notify([{ type: vscode.FileChangeType.Changed, uri: this.pathToUri(res.path) }]);
                    }
                } else {
            ...
            onRootDocUpdated: (_rootDocId: string) => {
                // intentionally no-op
            },
        });
    }

    pathToUri(...p: string[]): vscode.Uri {
        return vscode.Uri.joinPath(this.origin, ...p);
    }
    ...
    async resolve(uri: vscode.Uri): Promise<File> {
        ...
        switch (fileType) {
            case undefined: throw vscode.FileSystemError.FileNotFound(uri);
            case 'folder': return new File(fileName, vscode.FileType.Directory, undefined, readonly);
            case 'file':
    ...
    async list(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
        ...
        if (folder) {
            Object.values(FolderKeys).forEach((key) => {
                const t = key === 'folders' ? vscode.FileType.Directory : vscode.FileType.File;
                folder[key]?.forEach((e) => { results.push([e.name, t]); });
            });
        }
        return results;
    }
    ...
    async openFile(uri: vscode.Uri): Promise<Uint8Array> {
        ...
        if (!fileEntity) throw vscode.FileSystemError.FileNotFound();
    ...
    async createFile(uri: vscode.Uri, content: Uint8Array, overwrite?: boolean) {
        ...
        if (fileEntity && !overwrite) throw vscode.FileSystemError.FileExists(uri);
        ...
        if (content.length === 0) {
            ...
            if (_res.type === 'success') res = _res.entity;
        } else {
            ...
            if (_res.type === 'success' && _res.entity !== undefined) {
                res = _res.entity;
            } else if (_res.message !== undefined) {
        ...
        if (fileType === 'file' && fileEntity) {
            ...
            vscode.window.withProgress({
                ...
                if (res.type === 'success' && res.message !== undefined) {
                    fileEntity._id = res.message; // refresh entity id
    ...
    async createLinkedFile(uri: vscode.Uri) {
        ...
        const supportedProviders = [vscode.l10n.t('From Another Project'), vscode.l10n.t('From External URL')];
        const selection = await vscode.window.showQuickPick(supportedProviders, { placeHolder: vscode.l10n.t('Import file from...') });
        ...
        if (selection === vscode.l10n.t('From Another Project')) {
            ...
            const projectId = await vscode.window.showQuickPick(
                (await this.api.userProjectsJson(identity)).projects!
                    ...
                    .map(project => {
                        let detail = '';
                        for (const tag of allTags) if (tag.project_ids.includes(project.id)) detail += `$(tag) ${tag.name} `;
                        return { label: project.name, id: project.id, detail };
                    }),
                { title: vscode.l10n.t('Select a Project'), ignoreFocusOut: true }
            );
            const filePath = projectId && await vscode.window.showQuickPick(
                ...
                { title: vscode.l10n.t('Select a File'), ignoreFocusOut: true }
            );
            fileName = filePath && await vscode.window.showInputBox({
                ...
                ignoreFocusOut: true,
                validateInput: (value) => {
                    if (!value || value.match(/^[^\/?%*:|"<>]+$/g) === null) return vscode.l10n.t('File name is empty or contains invalid characters');
                    else if (parentFolder.fileRefs.find((fr) => fr.name === value) !== undefined) return vscode.l10n.t('A file or folder with this name already exists');
                }
            });
            data = { source_entity_path: filePath!, source_project_id: projectId!.id };
            const created = await (this.api as ExtendedBaseAPI).createLinkedFile(identity, this.projectId, parentFolder._id, fileName!, provider, data);
            if (created.type === 'success' && created.message !== undefined) entityId = created.message;
        } else if (selection === vscode.l10n.t('From External URL')) {
            provider = 'url';
            const url = await vscode.window.showInputBox({ title: vscode.l10n.t('URL to fetch the file from'), placeHolder: 'https://example.com/my-file.png', ignoreFocusOut: true });
        ...
        const entity = {
            ...
            linkedFileData: { provider, ...data! }, created: new Date().toISOString(),
        } as FileRefEntity;
        this.insertEntity(parentFolder, 'file', entity);
        const { path } = this._resolveById(entityId!)!;
        this.notify([{ type: vscode.FileChangeType.Created, uri: uri.with({ path: `/${this.projectName}${path}` }) }]);
    }

    /** Push the resolved merge to Overleaf (OT ops) and advance BASE (local/remote caches). */
    private async applyResolvedMerge(
        doc: DocumentEntity,
        destUri: vscode.Uri,
        mergedText: string
    ): Promise<void> {
        // Ensure numeric versions for UpdateSchema
        const v = typeof doc.version === 'number' ? doc.version : 0;
        const lastV = typeof doc.lastVersion === 'number' ? doc.lastVersion : v;

        // Compute ops from previous REMOTE -> merged
        const dmp = new DiffMatchPatch();
        const fromAscii = Buffer.from(doc.remoteCache ?? '', 'utf-8').toString('utf-8');
        const toAscii = Buffer.from(mergedText, 'utf-8').toString('utf-8');

        let p = 0;
        const op = dmp.diff_main(fromAscii, toAscii)
            .map((part) => {
                const start = p;
                if (part[0] === 0) { p += part[1].length; return undefined; }
                if (part[0] === 1) { p += part[1].length; return { p: start, i: part[1] }; }
                if (part[0] === -1) { return { p: start, d: part[1] }; }
            })
            .filter(Boolean) as any[];

        // Always compute the blob hash (avoid optional undefined)
        const hash = require('crypto')
            .createHash('sha1')
            .update(`blob ${mergedText.length}\x00${mergedText}`)
            .digest('hex');

        const update: UpdateSchema = { doc: doc._id, lastV, v, hash, op };

        // Send to Overleaf
        await this.socket.applyOtUpdate(doc._id, update);

        // Advance caches (BASE becomes merged)
        doc.localCache = mergedText;
        doc.remoteCache = mergedText;
        doc.lastVersion = v;
    ...
    async writeFile(uri: vscode.Uri, content: Uint8Array, create: boolean, overwrite: boolean) {
        ...
        if (!fileType && create) return this.createFile(uri, content, true);

        // if exists but not doc --> create new
        if (fileType && fileType !== 'doc' && create) return this.createFile(uri, content, overwrite);
    ...
    async remove(uri: vscode.Uri, recursive: boolean) {
        ...
        if (fileType && fileEntity) {
            ...
            if (res.type === 'success') {
                this.removeEntityById(parentFolder, fileType, fileEntity._id);
                this.notify([{ type: vscode.FileChangeType.Deleted, uri }]);
            } else if (res.message !== undefined) {
                vscode.window.showErrorMessage(res.message);
            }
        }
    ...
    async compile(force: boolean = false, draft: boolean = false, stopOnFirstError: boolean = false, rootDocId?: string) {
        if (force || (this.root && this.isDirty)) {
            ...
            try { await this.resolve(this.pathToUri(OUTPUT_FOLDER_NAME, 'output.log')); }
            catch { needCacheClearFirst = true; }
            const identity = await GlobalStateManager.authenticate(this.context, this.serverName);
            if (needCacheClearFirst) await this.api.deleteAuxFiles(identity, this.projectId);
            ...
            if (res.type === 'success' && res.compile?.status === 'success') {
                ...
                if (res.message !== undefined) console.error('Compile failure.', res.message);
                return false;
            }
        }
    ...
    async stopCompile() {
        ...
        if (res.type === 'success') return true;
            ...
            if (res.message !== undefined) vscode.window.showErrorMessage(res.message);
            return false;
        }
    }
    ...
    async updateOutputs(outputs: Array<OutputFileEntity>) {
        if (this.root) {
            ...
            if (this.removeEntityById(rootFolder, 'folder', __OUTPUTS_ID)) {
                this.notify([{ type: vscode.FileChangeType.Deleted, uri: this.pathToUri(OUTPUT_FOLDER_NAME) }]);
            }
